...
- Raw dynamic-tree traversal has an experimental Metal path, but CPU tree
  mutation/rebuild, prefixing, candidate filtering, and narrow phase remain.
...
1. Replace the intermediate CPU prefix with an on-device deterministic scan,
   then update or rebuild a GPU-owned broad-phase structure directly from
   resident shape bounds so the flat per-shape result stream and CPU tree copy
   can disappear.
