...
static b3WorldId CreateDistanceJointWorld( int bodyCount, int workerCount, bool enableMetal )
{
	...
	for ( int pair = 0; pair < bodyCount / 2; ++pair )
	{
		...
		bodyDefA.linearVelocity = (b3Vec3){ 0.02f, -0.01f, 0.015f };
		b3BodyDef bodyDefB = bodyDefA;
		bodyDefB.position.x += 1.2f;
		bodyDefB.linearVelocity = (b3Vec3){ -0.025f, 0.02f, -0.01f };
		...
		jointDef.lowerSpringForce = -50.0f;
		jointDef.upperSpringForce = 50.0f;
		jointDef.enableLimit = true;
		jointDef.minLength = 0.9f;
		jointDef.maxLength = 1.1f;
		jointDef.enableMotor = true;
		jointDef.maxMotorForce = 10.0f;
		jointDef.motorSpeed = ( pair & 1 ) ? 0.1f : -0.1f;
		b3CreateDistanceJoint( world, &jointDef );
	}
	return world;
}
...
int main( int argc, char** argv )
{
	...
	printf( "# operation=whole_world_distance_joints spring_limit_motor=on substeps=4 workers=%d timing=wall_clock_step\n",
		workerCount );
	...
	for ( int testIndex = 0; testIndex < (int)( sizeof( counts ) / sizeof( counts[0] ) ); ++testIndex )
	{
		...
		b3WorldId cpu = CreateDistanceJointWorld( bodyCount, workerCount, false );
		double cpuMs = TimeWorld( cpu, 8, repeats );
		b3DestroyWorld( cpu );
		b3WorldId gpu = CreateDistanceJointWorld( bodyCount, workerCount, true );
		if ( B3_IS_NULL( gpu ) )
		{
