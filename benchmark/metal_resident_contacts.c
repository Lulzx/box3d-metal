...
int main( void )
{
	...
	printf( "# operation=whole_world_resident_sphere_contacts substeps=4 workers=%d timing=wall_clock_step\n",
		workerCount );
	printf( "contacts,repeats,cpu_ms,gpu_ms,speedup,prepare_dispatches,index_bytes,prior_stream_bytes,impulse_bytes,prior_impulse_bytes\n" );
	for ( int testIndex = 0; testIndex < testCount; ++testIndex )
	{
		...
		printf( "%d,%d,%.6f,%.6f,%.3f,%llu,%llu,%llu,%llu,%llu\n", contactCount, repeats, cpuMs, gpuMs,
