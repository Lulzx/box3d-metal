...
The direct end-to-end oracle also covers 2,048 moving spheres, capsules, and
...
Three warm process runs were recorded for each mode. The table compares the
median Metal-world step time; the CPU reference timing is not used in this
within-Metal comparison because it showed scheduling variance across processes.
...
the following step. The stage remains experimental. GPU pair generation must
consume resident bounds and return only compact candidate/contact data before
this result stream can be removed.
