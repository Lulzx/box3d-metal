...
typedef struct b3World
{
	...
	uint64_t metalContactPrepareDispatchCount;
	...
	uint64_t metalLastAwakeIslandBitSetBytes;
	...
	uint64_t metalShapeResultGeneration;
	bool metalShapeCpuBoundsStale;
	...
	uint64_t metalResidentPairMoveDispatchCount;
	uint64_t metalEnlargedShapeTraversalBypassCount;
	...
	int metalLastResidentConvexContactCount;
	int metalLastResidentConvexConstraintCount;
...
static inline void b3BumpMetalBodyStateRevision( b3World* world )
{
#if defined( BOX3D_METAL )
	world->metalBodyStateRevision += 1;
	if ( world->metalBodyStateRevision == 0 )
		world->metalBodyStateRevision = 1;
#else
	B3_UNUSED( world );
#endif
