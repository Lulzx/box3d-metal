...
static int VerifyResidentPairTraversal( b3World* world )
{
	...
	if ( records == NULL )
	{
		...
		ENSURE( contactSeedCount == candidateCount );
...
static int MetalConvexManifoldTest( void )
{
	...
	int eligibleCount = 0;
	const b3MetalContactTransition* directTransitions = NULL;
	int directTransitionCount = 0;
	b3MetalDispatchStats stats = { 0 };
	ENSURE( b3MetalComputeConvexManifolds( world->metalContext, world, contactIndices, contactCount, &gpu, &eligibleCount, NULL,
										   &directTransitions, &directTransitionCount, &stats ) );
	ENSURE( directTransitionCount == 0 );
...
static int MetalPairTraversalTest( void )
{
	...
	ENSURE( gpuRecords == NULL );
	ENSURE( gpuCandidates == NULL );
	ENSURE( cpuFilterMoves == NULL );
	ENSURE( stats.pairPrivateScratchDispatchCount == 1 );
	ENSURE( stats.pairRawSharedBytes == 0 );
	...
	for ( int moveIndex = 0; moveIndex < moveCount; ++moveIndex )
	{
		...
		capture.queryShapeIndex = (int)b3DynamicTree_GetUserData( broadPhase->trees + proxyType, proxyId );
		capture.count = 0;
		if ( proxyType == b3_dynamicBody )
		{
			capture.treeType = b3_kinematicBody;
		...
		for ( int candidateIndex = 0; candidateIndex < capture.count; ++candidateIndex )
		{
			...
			const b3MetalPairContactSeed* seed = contactSeeds + totalCpuCount + capture.count - 1 - candidateIndex;
			ENSURE( cpu->shapeIndex == seed->shapeIndexA );
			ENSURE( capture.queryShapeIndex == seed->shapeIndexB );
		}
...
static int CompareTouchingGraphTopology( const b3World* cpu, const b3World* gpu )
{
	for ( int contactId = 0; contactId < cpu->contacts.count; ++contactId )
	{
		const b3Contact* a = cpu->contacts.data + contactId;
		const b3Contact* b = gpu->contacts.data + contactId;
		if ( a->contactId == B3_NULL_INDEX )
			continue;
		ENSURE( ( a->flags & b3_contactTouchingFlag ) != 0 );
		ENSURE( ( b->flags & b3_contactTouchingFlag ) != 0 );
		ENSURE( a->setIndex == b->setIndex );
		ENSURE( a->colorIndex == b->colorIndex );
		ENSURE( a->localIndex == b->localIndex );
		ENSURE( a->islandId == b->islandId );
		ENSURE( a->islandIndex == b->islandIndex );
		ENSURE( a->bodySimIndexA == b->bodySimIndexA );
		ENSURE( a->bodySimIndexB == b->bodySimIndexB );
	}

	for ( int colorIndex = 0; colorIndex < B3_GRAPH_COLOR_COUNT; ++colorIndex )
	{
		const b3GraphColor* a = cpu->constraintGraph.colors + colorIndex;
		const b3GraphColor* b = gpu->constraintGraph.colors + colorIndex;
		ENSURE( a->bodySet.blockCount == b->bodySet.blockCount );
		ENSURE( memcmp( a->bodySet.bits, b->bodySet.bits,
				(size_t)a->bodySet.blockCount * sizeof( uint64_t ) ) == 0 );
		ENSURE( a->convexContacts.count == b->convexContacts.count );
		ENSURE( memcmp( a->convexContacts.data, b->convexContacts.data,
				(size_t)a->convexContacts.count * sizeof( int ) ) == 0 );
	}
	return 0;
}
...
static int MetalFinalPairPlanOrderTest( void )
{
	...
	{
		const b3Contact* contact = gpu->contacts.data + i;
		if ( contact->contactId != i )
			continue;
		ENSURE( contact->manifoldCount == 1 );
		ENSURE( contact->manifolds == NULL );
		ENSURE( ( contact->flags & ( b3_simMetalManifold | b3_simMetalManifoldStale ) ) ==
				( b3_simMetalManifold | b3_simMetalManifoldStale ) );
	}

	// No CPU manifold exists until a public consumer asks for it. One GetData
	// call must materialize exactly that contact from the private resident table
	// without perturbing topology.
	int contactId = gpu->constraintGraph.colors[cpu->contacts.data[0].colorIndex].convexContacts.data[0];
	b3Contact* gpuContact = gpu->contacts.data + contactId;
	const b3Contact* cpuContact = cpu->contacts.data + contactId;
	ENSURE( ( gpuContact->flags & b3_simMetalManifold ) != 0 );
	ENSURE( ( gpuContact->flags & b3_simMetalManifoldStale ) != 0 );
	ENSURE( gpuContact->manifoldCount == 1 && gpuContact->manifolds == NULL );
	ENSURE( profile.contactManifoldSyncCount == 0 );
	b3ContactId publicContactId = { contactId + 1, gpu->worldId, 0, gpuContact->generation };
	b3ContactData publicData = b3Contact_GetData( publicContactId );
	ENSURE( publicData.manifoldCount == 1 && publicData.manifolds == gpuContact->manifolds );
	ENSURE( ( gpuContact->flags & b3_simMetalManifoldStale ) == 0 );
	profile = b3World_GetMetalProfile( gpuWorldId );
	ENSURE( profile.contactManifoldSyncCount == 1 );
	ENSURE( profile.residentContactNoCpuManifoldCount == (uint64_t)contactCount - 1u );
	ENSURE( cpuContact->manifoldCount == 1 );
	ENSURE( publicData.manifolds[0].pointCount == cpuContact->manifolds[0].pointCount );
	ENSURE( b3Length( b3Sub( publicData.manifolds[0].normal, cpuContact->manifolds[0].normal ) ) <= 1.0e-5f );
	for ( int pointIndex = 0; pointIndex < publicData.manifolds[0].pointCount; ++pointIndex )
	{
		const b3ManifoldPoint* a = cpuContact->manifolds[0].points + pointIndex;
		const b3ManifoldPoint* b = publicData.manifolds[0].points + pointIndex;
		ENSURE( b3Length( b3Sub( a->anchorA, b->anchorA ) ) <= 1.0e-5f );
		ENSURE( b3Length( b3Sub( a->anchorB, b->anchorB ) ) <= 1.0e-5f );
		ENSURE( fabsf( a->separation - b->separation ) <= 1.0e-5f );
		ENSURE( a->featureId == b->featureId );
	}
	b3ContactData repeatedData = b3Contact_GetData( publicContactId );
	ENSURE( repeatedData.manifolds == publicData.manifolds && repeatedData.manifoldCount == 1 );
	...
	printf( "    final pair plan contacts=%d direct=%d pairSeedBytes=%llu legacyInputSeedBytes=%llu inputPrivateBytes=%llu "
			...
			(size_t)contactCount * sizeof( b3MetalContactTransition ) );
...
static int MetalColdContactInputRecycleTest( void )
{
	...
	ENSURE( profile.contactInputPackCount == 0 );
	ENSURE( profile.lastContactTransitionCount == 3 );
	ENSURE( profile.lastContactTransitionBytes == 3u * sizeof( b3MetalContactTransition ) );
	ENSURE( profile.lastContactExceptionBytes == 0 );
	ENSURE( profile.lastContactCollisionExceptionCount == 0 );
	ENSURE( profile.contactCollisionCpuCount == 0 );
	ENSURE( profile.contactPrivateTopologyDispatchCount == 0 );
	...
	for ( int colorIndex = 0; colorIndex < B3_GRAPH_COLOR_COUNT; ++colorIndex )
	{
		...
		{
			// The creation input was 2,1,0 after LIFO recycling. Serial first-touch
			// topology must still consume the contact-state bitset in ascending ID order.
			ENSURE( color->convexContacts.data[i] == graphContactCount );
			graphContactCount += 1;
		}
	}
...
static int MetalColdContactInputFailureTest( void )
{
	...
	const b3MetalContactTransition* transitions = (const b3MetalContactTransition*)(uintptr_t)1;
	int transitionCount = -1;
	b3MetalDispatchStats stats = { 0 };
	ENSURE( b3MetalComputeConvexManifolds( world->metalContext, world, NULL, 1, &results, &resultCount,
			&residentBypassCount, &transitions, &transitionCount, &stats ) == false );
	ENSURE( results == NULL && resultCount == 0 && residentBypassCount == 0 );
	ENSURE( transitions == NULL && transitionCount == 0 );
	...
	ENSURE( b3MetalComputeConvexManifolds( world->metalContext, world, contactIndices, 1, &results, &resultCount,
			&residentBypassCount, &transitions, &transitionCount, &stats ) );
	ENSURE( results != NULL && resultCount == 1 && residentBypassCount == 0 );
	ENSURE( transitionCount == 0 );
	...
	transitions = (const b3MetalContactTransition*)(uintptr_t)1;
	transitionCount = -1;
	ENSURE( b3MetalComputeConvexManifolds( world->metalContext, world, NULL, 1, &results, &resultCount,
			&residentBypassCount, &transitions, &transitionCount, &stats ) == false );
	ENSURE( results == NULL && resultCount == 0 && residentBypassCount == 0 );
...
static int MetalFinalPairFilterExceptionTest( void )
{
	...
	ENSURE( profile.pairPrivateScratchDispatchCount == 0 );
	ENSURE( profile.lastPairRawSharedBytes > 0 );
	...
	// Eligibility is cached per shape revision and fails closed for the whole
	// world while compound or custom-filter shapes remain live.
	ENSURE( profile.pairPrivateScratchDispatchCount == 0 );
	ENSURE( profile.lastPairRawSharedBytes > 0 );
...
static int MetalPrivatePairDenseMaterializationTest( void )
{
	b3WorldDef worldDef = b3DefaultWorldDef();
	worldDef.gravity = b3Vec3_zero;
	b3WorldId cpuWorldId = b3CreateWorld( &worldDef );
...
static int MetalCompoundPairQueryGateTest( void )
{
	b3WorldDef worldDef = b3DefaultWorldDef();
	worldDef.gravity = b3Vec3_zero;
	b3WorldId worldId = b3CreateWorld( &worldDef );
	ENSURE( b3World_EnableMetal( worldId, 1 ) );
	b3SurfaceMaterial material = b3DefaultSurfaceMaterial();
	b3CompoundSphereDef child = {
		.sphere = { .center = b3Vec3_zero, .radius = 0.5f },
		.material = material,
	};
	b3CompoundDef compoundDef = { .spheres = &child, .sphereCount = 1 };
	b3CompoundData* compound = b3CreateCompound( &compoundDef );
	b3BodyDef bodyDef = b3DefaultBodyDef();
	b3BodyId compoundBody = b3CreateBody( worldId, &bodyDef );
	b3ShapeDef shapeDef = b3DefaultShapeDef();
	b3ShapeId compoundShape = b3CreateBakedCompoundShape( compoundBody, &shapeDef, compound );
	bodyDef.type = b3_dynamicBody;
	bodyDef.enableSleep = false;
	...
	b3World* world = b3GetWorldFromId( worldId );
	b3UpdateBroadPhasePairs( world );
	b3Filter filter = b3Shape_GetFilter( compoundShape );
	filter.groupIndex = 1;
	b3Shape_SetFilter( compoundShape, filter, true );
	ENSURE( world->broadPhase.moveArray.count == 1 );
	const b3MetalPairQueryRecord* records = NULL;
	const b3MetalPairCandidate* candidates = NULL;
	const int* cpuFilterMoves = NULL;
	const b3MetalPairContactSeed* contactSeeds = NULL;
	...
	int contactSeedCount = 0;
	b3MetalDispatchStats stats = { 0 };
	ENSURE( b3MetalGeneratePairCandidates( world->metalContext, world, world->broadPhase.moveArray.data, 1, &records,
			&candidates, &candidateCount, &cpuFilterMoves, &cpuFilterMoveCount, &contactSeeds, &contactSeedCount,
			&stats ) == false );
	ENSURE( records == NULL );
	ENSURE( candidates == NULL );
	ENSURE( cpuFilterMoves == NULL );
...
static int MetalResidentContactPrepareDifferentialTest( void )
{
	...
	ENSURE( profile.contactPrepareFallbackCount == 0 );
	ENSURE( profile.contactPrepareDeviceRefreshCount == 4 * count );
...
static int MetalResidentContactHitEventTest( void )
{
	...
	ENSURE( profile.lastContactCollisionExceptionCount == 1 );
	ENSURE( profile.lastContactTransitionCount == 0 );
	ENSURE( profile.lastContactTransitionBytes == 0 );
	ENSURE( profile.lastContactExceptionBytes == sizeof( b3MetalConvexManifoldResult ) );
	ENSURE( profile.contactCollisionCpuCount == 1 );
	ENSURE( profile.lastNarrowPhaseResultCount == 1 );
	ENSURE( profile.lastNarrowPhaseResultBytes == sizeof( b3MetalConvexManifoldResult ) );
	ENSURE( profile.contactPrepareDispatchCount == 1 );
...
static int MetalResidentCollisionBypassFallbackTest( void )
{
	...
	ENSURE( contact != NULL && contact->manifoldCount == 1 && contact->manifolds == NULL );
	ENSURE( b3World_GetMetalProfile( gpuWorld ).residentContactNoCpuManifoldCount == 1 );
	...
	ENSURE( profile.contactPrepareDispatchCount == 1 );
	ENSURE( profile.contactPrepareFallbackCount == 1 );
	ENSURE( profile.contactPrepareDeviceRefreshCount == 2 );
	ENSURE( profile.contactCollisionBypassCount == 1 );
	ENSURE( profile.contactCollisionCpuCount == 0 );
	ENSURE( profile.lastContactCollisionExceptionCount == 0 );
	ENSURE( profile.contactManifoldSyncCount == 1 );
	ENSURE( profile.residentContactNoCpuManifoldCount == 0 );
	ENSURE( contact->manifolds != NULL && b3IsContactManifoldStale( gpu, contact ) == false );
	float velocityError = b3Length( b3Sub( b3Body_GetLinearVelocity( cpuDynamic ), b3Body_GetLinearVelocity( gpuDynamic ) ) );
	ENSURE( velocityError <= 3.0e-5f );
...
static int MetalResidentCollisionBypassTransitionTest( void )
{
	...
	ENSURE( profile.contactCollisionBypassCount == 1 );
	ENSURE( profile.contactManifoldSyncCount == 0 );
	ENSURE( profile.residentContactNoCpuManifoldCount == 1 );
	ENSURE( contact->manifoldCount == 1 && contact->manifolds == NULL );
...
static int MetalDeferredManifoldLifecycleTest( void )
{
	b3WorldDef worldDef = b3DefaultWorldDef();
	worldDef.gravity = b3Vec3_zero;
	worldDef.enableSleep = false;
	...
	// Separation must remove a logical contact whose CPU manifold was never allocated.
	b3BodyId separatingBody = b3CreateBody( worldId, &dynamicDef );
	b3CreateSphereShape( separatingBody, &shapeDef, &sphere );
	b3World_Step( worldId, 1.0f / 60.0f, 1 );
	b3World* world = b3GetWorldFromId( worldId );
	b3Contact* contact = NULL;
	for ( int contactId = 0; contactId < world->contacts.count; ++contactId )
	...
	ENSURE( contact != NULL && contact->manifoldCount == 1 && contact->manifolds == NULL );
	ENSURE( b3World_GetMetalProfile( worldId ).residentContactNoCpuManifoldCount == 1 );
	b3Body_SetTransform( separatingBody, (b3Pos){ 4.0, 0.0, 0.0 }, b3Quat_identity );
	b3World_Step( worldId, 1.0f / 60.0f, 1 );
	ENSURE( world->broadPhase.pairSet.count == 0 );
	ENSURE( b3World_GetMetalProfile( worldId ).residentContactNoCpuManifoldCount == 0 );
	b3DestroyBody( separatingBody );

	// Destruction must also tolerate a live, null-backed logical manifold.
	b3BodyId destroyedBody = b3CreateBody( worldId, &dynamicDef );
	b3CreateSphereShape( destroyedBody, &shapeDef, &sphere );
	b3World_Step( worldId, 1.0f / 60.0f, 1 );
	contact = NULL;
	for ( int contactId = 0; contactId < world->contacts.count; ++contactId )
	{
	...
	ENSURE( contact != NULL && contact->manifoldCount == 1 && contact->manifolds == NULL );
	b3DestroyBody( destroyedBody );
	ENSURE( world->broadPhase.pairSet.count == 0 );
	ENSURE( b3World_GetMetalProfile( worldId ).residentContactNoCpuManifoldCount == 0 );

	// Recording snapshots are explicit CPU observation boundaries and materialize once.
	b3BodyId recordedBody = b3CreateBody( worldId, &dynamicDef );
	b3CreateSphereShape( recordedBody, &shapeDef, &sphere );
	b3World_Step( worldId, 1.0f / 60.0f, 1 );
	contact = NULL;
	for ( int contactId = 0; contactId < world->contacts.count; ++contactId )
	{
	...
	ENSURE( contact != NULL && contact->manifoldCount == 1 && contact->manifolds == NULL );
	b3MetalProfile beforeRecording = b3World_GetMetalProfile( worldId );
	b3Recording* recording = b3CreateRecording( 0 );
	ENSURE( recording != NULL );
	b3World_StartRecording( worldId, recording );
	ENSURE( contact->manifolds != NULL );
	b3MetalProfile afterRecording = b3World_GetMetalProfile( worldId );
	ENSURE( afterRecording.contactManifoldSyncCount == beforeRecording.contactManifoldSyncCount + 1 );
	ENSURE( afterRecording.residentContactNoCpuManifoldCount == 0 );
	b3World_StopRecording( worldId );
	b3DestroyRecording( recording );
	b3MetalInvalidateContactImpulseResults( world->metalContext );
	b3Recording* rejectedRecording = b3CreateRecording( 0 );
	ENSURE( rejectedRecording != NULL );
	b3World_StartRecording( worldId, rejectedRecording );
	ENSURE( world->recording == NULL );
	b3DestroyRecording( rejectedRecording );
	b3DestroyBody( recordedBody );
	printf( "    deferred manifold lifecycle separation=yes destruction=yes snapshotSync=1 snapshotReject=yes\n" );
	b3DestroyWorld( worldId );
	return 0;
}
...
int MetalTest( void )
{
	...
	RUN_SUBTEST( MetalPrivatePairDenseMaterializationTest );
	RUN_SUBTEST( MetalCompoundPairQueryGateTest );
